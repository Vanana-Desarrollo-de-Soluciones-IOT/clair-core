...
public class EdgeAlertController {
    ...
    public EdgeAlertController(AlertRepository repository) { this.repository = repository; }
    @GetMapping("/pending")
    ...
    public ResponseEntity<Void> acknowledge(@PathVariable UUID alertId, @Valid @RequestBody EdgeAlertAckRequest request) {
        return repository.findById(alertId).map(alert -> {
            if (alert.getStatus() == AlertStatus.ACKNOWLEDGED || alert.getStatus() == AlertStatus.RESOLVED) return ResponseEntity.status(HttpStatus.CONFLICT).<Void>build();
            if (alert.getStatus() != AlertStatus.ACTIVE || !repository.findHardwareIdByAlertId(alertId).map(request.hardware_id()::equals).orElse(false)) return ResponseEntity.notFound().<Void>build();
            alert.acknowledge(); repository.save(alert); return ResponseEntity.ok().<Void>build();
